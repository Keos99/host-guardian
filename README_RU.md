...
- Оповещения в чат о событиях мониторинга через настраиваемый HTTP webhook: падение и восстановление сервиса, попытки и ошибки перезапуска, исчерпание лимита рестартов, добавление и удаление сервисов, старт watcher.
- Управление оповещениями: главный выключатель в `application.yml`, глобальный runtime-переключатель и переключатель по каждому сервису на dashboard.
...
notification:
  chat:
    enabled: true        # главный выключатель функционала оповещений
    url: ""              # webhook чата; пусто = сообщения только в лог
    peer: ""             # идентификатор получателя (поле "peer" payload)
...
monitor:
  interval: 30s
  concurrency: 8 # сколько хостов проверять параллельно за один цикл
  ssh:
...
Host Guardian умеет отправлять сообщения о событиях мониторинга во внешний чат через HTTP webhook.

### События

| Событие | Статус | Когда отправляется |
|---|---|---|
| Watcher запущен | `SUCCESS` | при старте приложения, с количеством сервисов под мониторингом |
| Сервис добавлен | `SUCCESS` | при создании сервиса через API или dashboard |
| Сервис удален | `NOT_BUILT` | при удалении сервиса |
| Сервис недоступен | `FAIL` | один раз на эпизод недоступности (без спама каждые 30 секунд) |
| Попытка запуска | `UNSTABLE` | при каждой реально выполненной recovery-команде, с номером попытки в окне |
| Ошибка команды запуска | `FAILURE` | один раз на серию неудачных запусков |
| Исчерпан лимит рестартов | `FAILURE` | один раз, когда окно рестартов исчерпано и требуется ручное вмешательство |
| Ошибка мониторинга | `FAILURE` | один раз на эпизод, если сам мониторинг сервиса падает с ошибкой |
| Сервис восстановлен | `OK` | когда ранее упавший сервис снова прошел проверку |

### Транспорт

Сообщения отправляются POST-запросом на `notification.chat.url` в формате JSON:
...
1. `notification.chat.enabled=false` в `application.yml` — функционал выключен полностью, dashboard скрывает все элементы управления оповещениями.
2. Глобальный переключатель «Оповещения: вкл/выкл» на dashboard — хранится в базе (таблица `app_setting`) и переживает рестарт.
3. Переключатель конкретного сервиса — пункт меню действий на dashboard или чекбокс «Оповещения в чат» в форме сервиса.
...
- включение и отключение оповещений в чат глобально и по каждому сервису (элементы видны только при включенном `notification.chat.enabled`);
...
| `PATCH` | `/api/services/{id}/notifications?enabled=true|false` | Включить или отключить оповещения сервиса |
...
| Method | Path | Описание |
|---|---|---|
| `GET` | `/api/notifications` | Получить состояние функционала оповещений |
| `PATCH` | `/api/notifications?enabled=true|false` | Переключить глобальный runtime-выключатель (409, если функционал отключен в конфигурации) |
...
## Производительность при большом числе сервисов

Цикл проверки и запуска оптимизирован под десятки и сотни сервисов:

- **Один снимок процессов на хост.** Вместо отдельного `pgrep` на каждый сервис монитор один раз снимает список процессов (`ps -ww -eo pid=,args=`) и матчит `processMatch` локально. Отдельная проверка PID не нужна — в снимке только живые процессы.
- **Пакетный health-check.** HTTP-проверки всех сервисов удалённого хоста выполняются одним скриптом с параллельными `curl`, поэтому общее время ограничено самым медленным endpoint, а не их суммой.
- **Переиспользование SSH-подключения.** SSH-сессия открывается один раз на хост и переиспользуется между командами и циклами (`JschSessionPool`, провайдер `jsch`) с keepalive и переоткрытием при обрыве — полный SSH-хендшейк больше не выполняется на каждую команду. Для больших удалённых инсталляций рекомендуется провайдер `jsch`.
- **Параллельная проверка хостов.** Хосты независимы и проверяются параллельно через пул потоков размера `monitor.concurrency`. Внутри одного хоста работа последовательна, что сохраняет порядок recovery-действий.

В итоге на хост за цикл приходится порядка двух команд (снимок процессов + пакетный health) по одному переиспользуемому соединению — независимо от числа сервисов на хосте.
