...
|---|---|---|
| Watcher запущен | `INFO` | при старте приложения, с количеством сервисов под мониторингом |
| Сервис добавлен | `INFO` | при создании сервиса через API или dashboard |
| Сервис удален | `INFO` | при удалении сервиса |
| Сервис недоступен | `ALARM` | один раз на эпизод недоступности (без спама каждые 30 секунд) |
| Попытка запуска | `WARNING` | при каждой реально выполненной recovery-команде, с номером попытки в окне |
| Ошибка команды запуска | `ALARM` | один раз на серию неудачных запусков |
| Исчерпан лимит рестартов | `ALARM` | один раз, когда окно рестартов исчерпано и требуется ручное вмешательство |
| Ошибка мониторинга | `ALARM` | один раз на эпизод, если сам мониторинг сервиса падает с ошибкой |
...
Набор полей совместим со старым SberChat-отправителем: достаточно указать `url` и `peer`, чтобы переиспользовать существующий endpoint. Если `url` пуст, сообщения пишутся в лог приложения (log-only режим). Отправка выполняется асинхронно в отдельном потоке, поэтому медленный или недоступный чат не влияет на цикл мониторинга и REST-операции.
